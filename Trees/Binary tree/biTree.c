...
int attach(struct Node *child)
{
	...
	*/
	...
	{ // Infinate loop
		...
		{ // Checking for binary tree conditions
			if (parent->left == NULL)
			{
...
void search(int data)
{
	...
	*/
	...
	{ // Infinate loop
		if (current == NULL)
		{ // Checking if we have reached the leaf node
			printf("Not found..\n");
			break;
		}
		else if (current->data == data)
		{ // Comparing the node data with the arguement
			printf("Found!\n");
			break;
		}
		else if (data < current->data)
		{ // Refer point 2 in the above comment block
...
A:
	...
	{
	case '1':
	{ // the case where the user chooses to insert an element
		system("cls");
	...
	case '2':
	{ // The case where the user chooses to search for an element
		system("cls");
